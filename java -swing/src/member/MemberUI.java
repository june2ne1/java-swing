...
public class MemberUI extends JFrame {
	...
	public void init(){
		...
		title = new Vector<>(); // 테이블에 표시될 타이틀 백터
		title.add("아이디");
		title.add("이름");
		title.add("주소");
		Vector result = null; //MemberDAO가 selectAll() 한 결과를 받는 컬렉션
		//model.setDataVector(result, title);
		table = new JTable(model); //모델을 통해 테이블을 생성
		...
		btnAdd.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				String userid = txtUserid.getText();// 스캐너 의미
		...
		this.addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				...
				System.exit(0); // 스윙 문법으로 종료 처리 .. 알고리즘에서 return 의미
			}
